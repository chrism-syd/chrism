...
try {
  ...
  const designProducts = seed.products.filter((product) => product.productKind === 'christmas_card_design')
  ...
  assert.equal(
    ...
    christmasCatalog.CHRISTMAS_CARD_DESIGNS.length,
    'Each current CCiC card design maps to one internal non-sellable product'
  )
  assert.equal(
    boxProducts.length,
    christmasCatalog.CHRISTMAS_CARD_BOXES.length,
    'Each current CCiC 12-pack maps to one boxed product'
  )
  ...
  for (const product of designProducts) {
    assert.equal(product.priceCents, 0, `${product.title} design is not sold directly`)
    assert.equal(product.isPublic, false, `${product.title} design is internal only`)
    assert.equal(product.cardsPerBox, null, `${product.title} design does not carry box sizing`)
  }
  ...
  for (const product of boxProducts) {
    ...
    const components = seed.components.filter((component) => component.parentProductLegacyKey === product.legacyKey)
    assert.equal(components.length, 4, `${product.title} contains 4 card designs`)
    assert.equal(
      components.reduce((total, component) => total + component.quantity, 0),
      12,
      `${product.title} contains 12 cards total`
    )
    for (const component of components) {
      assert.equal(component.quantity, 3, `${product.title} contains 3 cards of each design`)
    }
  }
  ...
  assert.equal(classicCase.boxesPerCase, 35, 'Classic Sacred Case seed remains 35 boxes')

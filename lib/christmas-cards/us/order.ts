...
export const CCIC_US_ORDER_DRAFT_STORAGE_KEY = 'ccic-us-order-draft-v1'
...
export function parseCcicUsOrderDraftInput(value: unknown): CcicOrderDraftInput | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null

  const candidate = value as Partial<CcicOrderDraftInput>
  const fulfillmentMethod = 'shipping'
...
export function calculateCcicUsOrder(input: CcicOrderDraftInput): CcicCalculatedOrder {
  const normalizedInput = parseCcicUsOrderDraftInput(input) ?? {
    version: 1 as const,
    caseQuantities: {},
    boxQuantities: {},
  ...
  // is calculated by the isolated U.S. ShipTime rating endpoint once checkout is enabled.

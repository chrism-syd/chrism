...
export async function loadPublicInviteContext(
  ...
  const { data: eventData, error: eventError } = await supabase
    .from('events')
    .select(
      'id, local_unit_id, title, description, location_name, location_address, starts_at, ends_at, requires_rsvp, needs_volunteers, rsvp_deadline_at, status_code, scope_code'
    )
    .eq('id', invite.event_id)
    .maybeSingle();

  const rawEvent = eventData as ({ local_unit_id: string | null } & Omit<PublicInviteContext['event'], 'host_local_unit_id'>) | null;

-- Keep organization admin assignments and parallel area grants in sync.
-- This preserves current write paths while keeping the new permission rails current.
...
create or replace function public.trg_sync_parallel_area_grants_from_org_admin_assignment()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists organization_admin_assignments_sync_parallel_area_grants on public.organization_admin_assignments;
create trigger organization_admin_assignments_sync_parallel_area_grants
after insert or update on public.organization_admin_assignments
for each row execute function public.trg_sync_parallel_area_grants_from_org_admin_assignment();

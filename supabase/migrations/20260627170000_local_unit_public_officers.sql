...
create table if not exists public.local_unit_public_officers (
  ...
  constraint local_unit_public_officers_public_email_valid check (public_email is null or position('@' in public_email) > 1),
  constraint local_unit_public_officers_photo_bucket_path_pair check ((photo_storage_bucket is null and photo_storage_path is null) or (photo_storage_bucket is not null and photo_storage_path is not null)),
...
create unique index if not exists local_unit_public_officers_photo_storage_unique
  on public.local_unit_public_officers (photo_storage_bucket, photo_storage_path)
  where photo_storage_bucket is not null and photo_storage_path is not null;

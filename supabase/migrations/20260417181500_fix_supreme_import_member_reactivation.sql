...
comment on function public.apply_supreme_import_row(
  ...
  text,
  text
) is 'Applies one Supreme import row atomically, preferring the existing membership number match inside the current organization, reactivating the resolved person for the active local-unit surface when needed, and ensuring the resolved person is linked into the current local-unit member_records surface.';

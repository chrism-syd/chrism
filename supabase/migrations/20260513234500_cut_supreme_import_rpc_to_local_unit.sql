...
-- Move Supreme import from council-shaped operational scope to explicit local_unit_id scope.
-- people.council_id remains populated from the local unit's legacy council bridge for
-- Knights compatibility/routing only.

drop function if exists public.apply_supreme_import_row(
  uuid,
  uuid,
  uuid,
  text,
...
create function public.apply_supreme_import_row(
...
comment on function public.apply_supreme_import_row(
  ...
  text,
  text
) is 'Applies one Supreme import row atomically using explicit local_unit_id as operational scope, while preserving people.council_id as the Knights legacy compatibility bridge.';

revoke all on function public.apply_supreme_import_row(
  uuid,
  uuid,
  uuid,
  text,
...
grant execute on function public.apply_supreme_import_row(
  uuid,
  uuid,
  uuid,
  text,

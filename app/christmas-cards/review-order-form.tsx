...
export default function ReviewOrderForm() {
  ...
  const requestShippingRate = useCallback(async (postalCode: string) => {
    ...
    try {
      const response = await fetch('/api/ccic/shipping/rates', {
        ...
        body: JSON.stringify({ postalCode: compactCanadianPostalCode(postalCode), totalBoxes: calculatedOrder.totalSelectedBoxes }),
      })
      const payload = await response.json().catch(() => null) as { available?: boolean; message?: string; rate?: { amountCents?: number; serviceName?: string } } | null

      if (response.ok && payload?.available && typeof payload.rate?.amountCents === 'number') {
        setShipping({ status: 'priced', amountCents: payload.rate.amountCents, serviceName: payload.rate.serviceName || 'Canada Post' })
        return
      }
  ...
  async function submitOrder(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = await response.json().catch(() => null) as { error?: string; orderNumber?: string; confirmationEmailSent?: boolean } | null
    if (!response.ok || !payload?.orderNumber) { setError(payload?.error || 'We could not submit your order. Please try again.'); setSubmitting(false); return }
    window.sessionStorage.removeItem(CCIC_ORDER_DRAFT_STORAGE_KEY)
    setResult({ orderNumber: payload.orderNumber, confirmationEmailSent: Boolean(payload.confirmationEmailSent) })
    setSubmitting(false)
  }

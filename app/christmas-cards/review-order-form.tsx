...
const MANUAL_SHIPPING_MESSAGE = 'Shipping will be calculated after your order has been reviewed for packing.'
...
function addressFromForm(form: HTMLFormElement, postalCode: string): CcicSelectedAddress { const data = new FormData(form); return { addressLine1: fieldValue(data, 'address_line_1'), city: fieldValue(data, 'city'), province: fieldValue(data, 'province'), postalCode } }
...
function transitLabel(days: number | null) { return days === null ? null : `${days} business day${days === 1 ? '' : 's'}` }
...
export default function ReviewOrderForm() {
  ...
  const requestShippingRate = useCallback(async (address: CcicSelectedAddress) => {
    ...
    if (lastShippingRequestRef.current === requestKey) return
    ...
    try {
      ...
      if (response.ok && payload?.available && typeof payload.rate?.amountCents === 'number') { setShipping({ status: 'priced', amountCents: payload.rate.amountCents, serviceName: payload.rate.serviceName || 'Canada Post', transitDays: typeof payload.rate.expectedTransitTime === 'number' ? payload.rate.expectedTransitTime : null }); return }
  ...
  const handleAutocompleteAddressSelected = useCallback((selected: CcicSelectedAddress | string) => {
    revealAddressFields()
    if (typeof selected === 'string') { const form = document.querySelector<HTMLFormElement>('form.ccic-review-form'); if (!form || !isCanadianPostalCode(selected)) { setShipping({ status: 'waiting' }); return }; void requestShippingRate(addressFromForm(form, selected)); return }

...
type SubmissionResult = { orderNumber: string; confirmationEmailSent: boolean }
type ShippingState =
  | { status: 'waiting' }
  | { status: 'calculating' }
...
function addressFromForm(form: HTMLFormElement, postalCode: string): CcicSelectedAddress {
  const data = new FormData(form)
  return {
    addressLine1: fieldValue(data, 'address_line_1'),
    city: fieldValue(data, 'city'),
    province: fieldValue(data, 'province'),
    postalCode,
  }
}

function shippingRequestKey(address: CcicSelectedAddress, totalBoxes: number) {
  return [
    address.addressLine1.trim().toUpperCase(),
    address.city.trim().toUpperCase(),
    address.province.trim().toUpperCase(),
    compactCanadianPostalCode(address.postalCode),
    totalBoxes,
  ].join('|')
}
...
export default function ReviewOrderForm() {
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => setDraftInput(readStoredDraft()), 0)
  ...
  const requestShippingRate = useCallback(async (address: CcicSelectedAddress) => {
    if (!calculatedOrder || !isCanadianPostalCode(address.postalCode) || !address.addressLine1 || !address.city || !address.province) {
      lastShippingRequestRef.current = null
    ...
    try {
      const response = await fetch('/api/ccic/shipping/rates', {
        ...
        body: JSON.stringify({
          postalCode: compactCanadianPostalCode(address.postalCode),

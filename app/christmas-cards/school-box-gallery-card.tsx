...
export default function SchoolBoxGalleryCard({
  box,
  quantityLabel,
  quantity,
  onQuantityChange,
  ...
  return (
    <article className={`ccic-gallery-card ccic-school-gallery-card ${quantity > 0 ? 'is-selected' : ''}${isSoldOut ? ' is-sold-out' : ''}`}>
      <CardArt
        title={box.title}
      ...
      <div className="ccic-gallery-copy">
        ...
        <p className="ccic-school-box-description">12 cards of the same design + 12 envelopes</p>
        <strong className="ccic-school-box-price">{formatChristmasCardMoney(box.priceCents)}</strong>
      </div>
      {isSoldOut ? (

...
function setFormField(name: string, value: string) { if (!value) return; const field = document.querySelector<HTMLInputElement>(`input[name="${name}"]`); if (!field) return; field.value = value; field.dispatchEvent(new Event('input', { bubbles: true })); field.dispatchEvent(new Event('change', { bubbles: true })) }
...
export default function GoogleAddressAutocomplete({ onAddressSelected, onUnavailable }: GoogleAddressAutocompleteProps) {
  ...
  useEffect(() => {
    ...
    function initialize() {
      ...
      if (!Ctor) { attempts += 1; if (attempts <= 20) { retryTimer = window.setTimeout(initialize, 100); return }; console.error('CCIC Google address autocomplete failed to initialize: Google Places library is unavailable.'); showFallback('Address search is unavailable. Please enter your shipping address below.'); return }
      readyRef.current = true; setStatus(''); autocomplete = new Ctor(); autocomplete.includedRegionCodes = ['ca']; autocomplete.placeholder = 'Start typing your Canadian address'; autocomplete.setAttribute('aria-label', 'Search for your Canadian shipping address'); autocomplete.style.width = '100%'
      autocomplete.addEventListener('gmp-select', async (event) => {
        try {
          const prediction = (event as PlaceSelectEvent).placePrediction; if (!prediction) return
          const place = prediction.toPlace(); await place.fetchFields({ fields: ['addressComponents'] }); const components = place.addressComponents || []
    ...
    initialize(); return () => { disposed = true; if (retryTimer !== null) window.clearTimeout(retryTimer); if (autocomplete?.parentNode === host) host.removeChild(autocomplete) }
  }, [scriptReady])

...
function readStoredDraft() {
  const storedDraft = window.sessionStorage.getItem(CCIC_US_ORDER_DRAFT_STORAGE_KEY)
  if (!storedDraft) return null
  try { return parseCcicUsOrderDraftInput(JSON.parse(storedDraft)) } catch { return null }
}
...
export default function StorefrontOrderBuilder({ cases, boxes, collections, inventoryAvailability, caseAvailability }: Props) {
  ...
  const { isOpen, closeCart, setSummary } = useUsCcicCart()
  ...
  const calculatedOrder = useMemo(() => calculateCcicUsOrder(draftInput), [draftInput])
  ...
  function removeDisplayedBoxQuantity(catalogId: string, quantity: number) {
    setBoxQuantities((current) => setQuantityValue(current, catalogId, Math.max(0, quantityFromMap(current, catalogId) - quantity)))
  }
  ...
  useEffect(() => {
    ...
    if (calculatedOrder.hasOrder) window.sessionStorage.setItem(CCIC_US_ORDER_DRAFT_STORAGE_KEY, JSON.stringify(draftInput))
    else window.sessionStorage.removeItem(CCIC_US_ORDER_DRAFT_STORAGE_KEY)
              ...
              return (
                <article className="ccic-featured-case-card" key={item.id}>
                  <div className="ccic-classic-case-image">
                    <Image src="/CCIC_Classic32.jpg" alt={`${item.title} assortment of 32 Christmas card boxes`} fill sizes="(max-width: 860px) 100vw, 62vw" loading="eager" />
                  </div>
                  <div className="ccic-featured-case-copy">
                    <p className="ccic-eyebrow">Most popular</p><h2>{item.title}</h2><p>{item.description}</p>
                    <p className="ccic-featured-case-price">{formatUsChristmasCardMoney(item.priceCents)} per case <span className="ccic-unit-price">({formatUsChristmasCardMoney(Math.round(item.priceCents / item.boxesPerCase))} per box)</span></p>
                    <ul className="ccic-plain-list"><li>{item.boxesPerCase} boxed greeting card sets</li><li>12 cards and 12 envelopes per box</li><li>{item.boxesPerCase * 12} cards total</li><li>Preselected assortment containing 2 boxes of each of our 16 designs.</li><li><strong>Best value!</strong></li></ul>
                    {maxQuantity > 0 ? <QuantityControl label={`${item.title} cases`} value={quantity} max={maxQuantity} onChange={(value) => setCaseQuantities((current) => setQuantityValue(current, item.id, value))} /> : <span className="ccic-sold-out-pill" role="status">Sold out</span>}
                  </div>
                </article>
              )
            })}
                  ...
                  {collection.id === 'catholic-prayer-cards' && christmasSeals ? (
                    <section className="ccic-seals-banner" aria-labelledby="christmas-seals-title">
                      ...
                      <div className="ccic-seals-copy">
                        ...
                        <strong>{formatUsChristmasCardMoney(christmasSeals.priceCents)} per sheet</strong>

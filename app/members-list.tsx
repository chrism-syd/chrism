...
export default function MembersList({
  ...
  async function handleExportExcel() {
    ...
    const exportRows = filteredAndSortedMembers.map((member) => {
    ...
    XLSX.writeFile(workbook, `${fileSafe(`members-${today}`)}.xlsx`)

    setNotice({ tone: 'success', text: `Exported ${exportRows.length} members to Excel.` })
  }

  async function handleCopyEmails() {
    const recipients = getUniqueEmailRecipients(filteredAndSortedMembers)

    if (recipients.length === 0) {
      setNotice({ tone: 'error', text: 'No email addresses are available in this filtered view.' })
      return
    }
    ...
    try {
      ...
      setNotice({ tone: 'success', text: `Copied ${recipients.length} email address${recipients.length === 1 ? '' : 'es'} for your email client.` })
    } catch {
  ...
  function handleCreateList() {
    ...
    if (filteredAndSortedMembers.length === 0) {
      setNotice({ tone: 'error', text: 'There are no members in this filtered view to save into a custom list.' })
      return
    }
    ...
    <>
      <section className="qv-card qv-members-list-card">
        <div className="qv-directory-section-head">
          <div>
            <h2 className="qv-section-title">Member listing</h2>
            <p className="qv-section-subtitle">Search, sort, and manage member records.</p>
          </div>

          <div className="qv-view-menu-stack">
            <details className="qv-view-menu" ref={viewMenuRef}>
              <summary>
                <span>Use Current View</span>
                <span aria-hidden="true" className="qv-view-menu-chevron">▾</span>
              </summary>
              <div className="qv-view-menu-panel">
                <button type="button" className="qv-view-menu-item" onClick={handleCreateList}>
                  Create Custom List
                </button>
                <button
                  type="button"
          ...
          <div className="qv-member-table-scroll">
            <div className="qv-member-list">
              {paginatedMembers.map((person) => {
                ...
                return (
                  <Link key={person.id} href={`/members/${person.id}`} className="qv-member-link">
                    <div className="qv-member-row qv-member-row-compact">
      ...
      {createListOpen ? (
        <CreateCustomListDialog
          ...
          memberIds={filteredAndSortedMembers.map((member) => member.id)}
          previewNames={filteredAndSortedMembers.slice(0, 12).map((member) => `${member.first_name} ${member.last_name}`.trim())}
        />
      ) : null}
    </>
  )

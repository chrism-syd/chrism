...
## Local maintenance helper scripts

It is acceptable to use small local scripts for broad, mechanical refactors when connector edits would be slow, fragile, or likely to collide with a large file.

Preferred pattern:

- place reusable helpers under `scripts/maintenance/`
- keep one-off scratch helpers out of commits unless they document a repeatable migration pattern
- make scripts idempotent where practical
- print exactly which files were changed
- run `git diff`, `npm run verify`, and `npm run build` before committing
- commit scripts separately or with a clear refactor commit only when they are useful future maintenance tooling

This is especially useful for local-unit/council cleanup work, generated replacements, migration scaffolding, or other high-confidence mechanical edits. Do not use helper scripts as a substitute for understanding the model; use them as a safe way to apply a well-understood change.
